<?php
 ...
 $query = "SELECT * FROM `qb-garages-config-locations`";
 $garloc = sqlrun($query);
 ...
 foreach($garloc as $k => $v){
  if($v['enabled'] === 'true'){
   $output .= " [\"{$v['name']}\"] = {[\"label\"] = \"{$v['label']}\",[\"blipName\"] = \"{$v['blipname']}\",[\"showBlip\"] = {$v['showblip']},[\"blipNumber\"] = {$v['blipnumber']},[\"blipColor\"] = {$v['blipcolor']},[\"takeVehicle\"] = {$v['takevehicle']},[\"spawnPoint\"] = {$v['spawnpoint']},[\"type\"] = \"{$v['type']}\",[\"vehicle\"] = \"{$v['vehicle']}\"";
   if($v['putvehicle']){
